...
{
    public class BrickBit : GenericBlock
    {
        private float yVelocity;
        private float gravity = SceneUtilityConsts.Gravity;
        private float xVelocity;
        private int timer = 0;
        public BrickBit(float x, float y, Texture2D spritesheet, float xVel, float yVel, Func<int, int, IItem> func) : base(spritesheet, func)
        {
            IsVisible = true;
            XPosition = x;
            YPosition = y;
            yVelocity = yVel;
            xVelocity = xVel;
            SpriteByFrame.Add(0, SceneUtilityConsts.BrickBitRectangle);
            FrameDelay = 1;
            TotalFrames = 1;
            Depth = 1;
        }
